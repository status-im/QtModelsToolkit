...
Item {
    ...
    readonly property int numberOfTokens: 300
    ...
    function getByKey(model, keyRole, keyValue) {
        const idx = ModelQuery.indexOf(model, keyRole, keyValue)

        if (idx === -1)
            return null

        return ModelQuery.get(model, idx)
    }
                        ...
                        Repeater {
                            ...
                            Rectangle {
                                ...
                                MouseArea {
                                    ...
                                    onClicked: {
                                        const item = root.getByKey(
                                                       tokensModel, "name", tokenLabel.text)

                                        const index = ModelQuery.indexOf(
