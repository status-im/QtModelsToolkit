...
/*!
    \qmltype MovableModel
    \inherits QAbstractListModel
    \inqmlmodule QtModelsToolkit
    \ingroup ProxyModels
    \brief Proxy model allowing freely reorder rows without modifying the source model.
*/
...
/*!
    \qmlmethod void MovableModel::desyncOrder()

    Explicitly desynchronize order. It means that reorders within source model
    won't be reflected in the proxy.
*/
...
/*!
    \qmlmethod void MovableModel::syncOrder()
...
/*!
    \qmlmethod void MovableModel::move(from, to, count)
...
/*!
    \qmlmethod list<int> MovableModel::order()

    Returns list where every entry indicates position of given index of a proxy
    in a source model.
*/
...
/*!
    \qmlproperty bool MovableModel::synced

    This read-only properties specifies if the rows order of the proxy is
    synced with the rows order of the source model. When true, rows moves of the
    source model are directly reflected in the proxy model.

    Synced property becomes false when \l move or \l desyncOrder are called.
*/
