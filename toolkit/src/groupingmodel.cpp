...
/*!
    \qmltype GroupingModel
    \inherits QAbstractProxyModel
    \inqmlmodule QtModelsToolkit
    \ingroup ProxyModels
    \brief Proxy model grouping rows into submodels by value of given role.

    The proxy groups consecutive rows with the same value of role
    \l groupingRoleName into submodel and exposed under role name
    \l submodelRoleName. Only the first row from the group remains in the
    top-level model (with additional role holding the grouped submodel).
*/
...
/*!
    \qmlproperty string GroupingModel::groupingRoleName

    This property holds the role name that the sorter is proxy is using group
...
/*!
    \qmlproperty string GroupingModel::submodelRoleName

    This property holds the role name that is used to expose the submodel
    with groupped rows.
*/
